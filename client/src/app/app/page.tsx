...
export default function AppDashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError("");
      try {
        const [dealsData, walletData, notifData, timeLineData] = await Promise.all([
  ...
  const totalVolumePKR = timeLine.reduce((acc, curr) => acc + curr.moneyMinor / 100, 0);
  ...
  const getStatusBadge = (status: string) => {
    const map: Record<string, string> = {
      agreed: "bg-red-50 text-red-700 border-red-200 dark:bg-red-500/10 dark:text-red-300 dark:border-red-500/30",
      funded: "bg-red-50 text-red-700 border-red-200 dark:bg-red-500/10 dark:text-red-300 dark:border-red-500/30",
  ...
  return (
    <div className="space-y-6 max-w-7xl  px-4 sm:px-6 lg:px-8 pb-8 text-foreground">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5">
        <div className="bg-background border border-border-color rounded-sm text-center p-3 sm:p-4">
          ...
          <div className="text-2xl font-bold italic text-foreground mt-1.5">
            {activeDeals.length}
          </div>
          <p className="text-[13px] text-foreground mt-1">In progress & pending</p>
        </div>
        ...
        <div className="bg-background border border-border-color rounded-sm text-center p-3 sm:p-4">
          ...
          <p className="text-[13px] text-foreground mt-1">Available for payout</p>
        </div>
        ...
        {wallet?.escrowMinor > 0 && (
          <div className="bg-background border border-border-color rounded-sm text-center p-3 sm:p-4">
            ...
            <p className="text-[13px] text-foreground mt-1">Held until delivery</p>
          </div>
        )}

        <div className="bg-background border border-border-color rounded-sm text-center p-3 sm:p-4 col-span-2 sm:col-span-1 md:col-span-1 lg:col-span-1">
          ...
          <p className="text-[14px] text-foreground mt-1">Across completed deals</p>
        </div>
      </div>

      {/* ── Earnings/Spending Chart ── */}
      <div className="bg-background border border-border-color rounded-sm p-5 sm:p-6 lg:p-8 relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 relative z-10">
          <div>
            ...
            <p className="text-xs sm:text-sm text-zinc-500 mt-0.5">Performance tracking over the last 12 months</p>
          </div>
          ...
          <div className="self-start sm:self-auto px-3.5 py-1.5 rounded-sm bg-surface border border-border-color flex items-center gap-3">
            <div>
              <span className="text-[10px] sm:text-xs font-medium text-zinc-500 block italic">Total {isBrand ? "Spent" : "Earned"}</span>
              <p className="text-xs sm:text-sm font-black text-foreground">
                PKR {chartData.reduce((acc, curr) => acc + (curr.moneyPKR || 0), 0).toLocaleString()}
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Chart Area */}
        <div className="w-full h-[280px] sm:h-[320px] lg:h-[360px] relative z-10 text-foreground">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData} margin={{ top: 15, right: 10, left: -20, bottom: 0 }}>
              ...
              <CartesianGrid strokeDasharray="4 4" stroke="currentColor" strokeOpacity={0.12} vertical={false} />
              ...
              <Tooltip
                ...
                content={({ active, payload, label }) => {
                  ...
                  return (
                    <div className="bg-background border border-border-color rounded-2xl p-3.5 text-xs shadow-xl text-foreground space-y-1.5 min-w-[170px]">
                      <div className="flex items-center justify-between border-b border-border-color pb-1.5">
                        <span className="font-bold text-foreground">{label}</span>
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ${isUp ? "bg-surface text-foreground" : "bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-300"}`}>
                          {isUp ? `+${diff.toLocaleString()}` : `${diff.toLocaleString()}`}
                        </span>
                      </div>

                      <div className="pt-0.5">
                        <span className="text-[10px] text-zinc-500 block font-medium">Amount</span>
                      ...
                      <div className="flex items-center justify-between pt-1 border-t border-border-color text-[11px] text-foreground">
                        ...
                        <span className="font-bold text-foreground">{current.deals}</span>
                      </div>
                    </div>
                  );
                }}
              ...
              <Line
                ...
                activeDot={{ r: 7, fill: "#dc2626", stroke: "#ffffff", strokeWidth: 2.5 }}
              />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-5 sm:gap-6">
        {/* Recent Deals */}
        <div className="bg-background border border-border-color rounded-sm p-5 sm:p-6 lg:p-8 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                <h2 className="text-sm sm:text-base font-semibold text-foreground">Recent Deals</h2>
              </div>
              <Link
                href="/app/deals"
            ...
            {deals.length === 0 ? (
              <div className="p-8 text-center bg-surface border border-border-color rounded-2xl">
                <p className="text-xs sm:text-sm text-zinc-500">No active or historical deals yet.</p>
              </div>
            ) : (
              <div className="space-y-2.5">
                {deals.slice(0, 5).map((d) => (
                  <Link
                    ...
                    className="flex items-center justify-between p-3.5 rounded-sm bg-surface border border-border-color hover:border-red-400/60 hover:bg-red-500/5 transition-all duration-200 group"
                  >
                    <div className="min-w-0 pr-2">
                      <p className="text-xs sm:text-sm font-bold text-foreground group-hover:text-red-600 transition truncate">
                        {d.title}
                      </p>
                      <p className="text-[11px] sm:text-xs font-semibold text-zinc-500 mt-0.5">{money(d.priceMinor)}</p>
                    </div>
                    <span
        ...
        <div className="bg-background border border-border-color rounded-sm p-5 sm:p-6 lg:p-8 flex flex-col justify-between">
          <div>
            ...
            {notifications.length === 0 ? (
              <div className="p-8 text-center bg-surface border border-border-color rounded-2xl">
                <p className="text-xs sm:text-sm text-zinc-500">No recent notifications.</p>
              </div>
            ) : (
              <div className="space-y-2.5">
                {notifications.map((n) => (
