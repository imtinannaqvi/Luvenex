...
export default function CreatorProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-[#B90808] selection:text-foreground py-8 sm:py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      {/* ambient red glows */}
      ...
      <div className="relative z-10 max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-6 lg:gap-8 items-start">
        ...
        <aside className="bg-card border border-border-color rounded-md p-6 lg:sticky lg:top-8 space-y-6">
          ...
          <div className="flex flex-col items-center text-center space-y-3">
            ...
            <div className="flex items-center gap-1.5">
              <h1 className="text-2xl font-black text-foreground">{profile.title || profile.handle}</h1>
              {profile.isVerified && (
          ...
          <div className="flex items-center justify-center gap-4 text-sm">
            {avgRating && (
              <span className="flex items-center gap-1 text-foreground font-semibold">
                ...
                <span className="text-foreground">({reviews.length})</span>
              </span>
            )}
          ...
          {profile.socialAccounts && profile.socialAccounts.length > 0 && (
            <div className="flex flex-wrap gap-2 justify-center pt-2 border-t border-border-color">
              {profile.socialAccounts.map((acc: any, i: number) => {
                ...
                const content = (
                  <div className="flex items-center gap-1.5 bg-background border border-border-color rounded-full pl-2 pr-3 py-1.5 hover:border-[#B90808]/60 transition">
                    <span className="w-5 h-5 rounded-full bg-background flex items-center justify-center text-foreground shrink-0">
                      <Icon size={11} />
                    </span>
                    <span className="text-[11px] font-semibold text-foreground">@{acc.handle}</span>
                  </div>
                );
                return acc.url ? (
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-background border border-border-color rounded-md py-1 text-center">
              <span className="block text-md font-black text-foreground">{videos.length}</span>
              <span className="text-[11px] italic text-zinc-500 font-medium">Videos</span>
            </div>
            <div className="bg-background border border-border-color rounded-md py-1 text-center">
              <span className="block text-md font-black text-foreground">{followerCount}</span>
              <span className="text-[11px] italic text-foreground font-medium">Followers</span>
            </div>
            <div className="bg-background border border-border-color rounded-md py-1 text-center">
              <span className="block text-md font-black text-foreground">{followingCount}</span>
              <span className="text-[11px] italic text-foreground font-medium">Following</span>
            </div>
          </div>
          ...
          <div className="space-y-2.5">
            ...
            <button
              ...
              className="w-full py-3 rounded-md bg-background hover:bg-surface text-foreground text-sm font-bold border border-border-color transition active:scale-95"
            >
              Message
            </button>
          </div>
        ...
        <section className="space-y-5">
          {/* tab bar */}
          <div className="flex flex-wrap items-center gap-1 bg-surface border border-border-color rounded-md p-1.5">
            {TABS.map((tab) => (
              <button
                key={tab.id}
          ...
          {/* content card */}
          <div className="bg-surface border border-border-color rounded-md p-6 sm:p-8 min-h-[400px]">
            ...
            {activeTab === "about" && (
              <div className="space-y-6">
                ...
                {profile.bio ? (
                  ...
                  <p className="text-sm text-foreground">No bio added yet.</p>
                )}

                {profile.niches && profile.niches.length > 0 && (
                  <div className="flex flex-wrap gap-2 pt-2">
                    {profile.niches.map((n: string) => (
                      <span key={n} className="text-xs text-foreground font-semibold bg-background border border-border-color rounded-md px-3 py-1">
                        #{n}
                      </span>
                    ))}
                  </div>
            ...
            {activeTab === "feedback" && (
              <div className="space-y-4">
                ...
                {reviews.length === 0 ? (
                  <div className="text-center py-12 bg-background border border-dashed border-border-color rounded-2xl">
                    <p className="text-sm text-foreground">No feedback yet.</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {reviews.map((r) => (
                      <div key={r._id} className="bg-background border border-border-color rounded-md p-4 space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-bold text-foreground">{r.reviewerId?.name || "Anonymous Client"}</span>
            ...
            {activeTab === "history" && (
              <div className="space-y-4">
                <h2 className="text-xl font-black text-foreground">Work history</h2>
                {workHistory.length > 0 ? (
                  <div className="space-y-3">
                    {workHistory.map((w, i) => (
                      <div
                        key={i}
                        className="flex justify-between items-center bg-background border border-border-color rounded-md px-4 py-3"
                      >
                        <div>
                  ...
                  <div className="text-center py-12 bg-background border border-dashed border-border-color rounded-md">
                    <p className="text-sm text-foreground">No work history added yet.</p>
                    <p className="text-xs text-foreground mt-1">Completed jobs will appear here.</p>
                  </div>
                )}
              </div>
            )}
  ...
  <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {profile.portfolio
          ...
          .map((item: any) => (
            <div
              key={item._id}
              className="relative rounded-md overflow-hidden border border-border-color bg-background aspect-square group"
            >
              ...
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-background via-background to-transparent p-3 pt-8 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <p className="text-xs font-bold text-foreground truncate">{item.title}</p>
                {item.description && (
                  <p className="text-[11px] text-foreground truncate mt-0.5">{item.description}</p>
                )}
              </div>
            </div>
          ))}
    ...
    {gigs.length > 0 && (
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {gigs.map((g) => (
          <div key={g._id} className="bg-surface border border-border-color rounded-md p-4 space-y-2.5">
            <div className="space-y-1">
              <span className="inline-block text-[13px] font-bold text-[#B90808] ">
                {g.category || "General"}
              </span>
              <h3 className="text-sm font-bold text-foreground leading-snug line-clamp-2">{g.title}</h3>
    ...
    {(!profile?.portfolio || profile.portfolio.filter((item: any) => item.mediaUrl).length === 0) && gigs.length === 0 && (
      <div className="text-center py-12 bg-surface/40 border border-dashed border-border-color rounded-md">
        <p className="text-sm text-zinc-500 mb-3">No portfolio items yet.</p>
            ...
            {activeTab === "skills" && (
              <div className="space-y-4">
                <h2 className="text-md font-black text-foreground">Skills</h2>
                {profile.niches && profile.niches.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {profile.niches.map((n: string) => (
                      <span key={n} className="text-sm text-foreground font-semibold bg-surface border border-border-color rounded-md px-4 py-2 hover:border-[#B90808]/60 transition">
                        {n}
                      </span>
                    ))}
                  </div>
                ) : (
                  <div className="text-center py-12 bg-surface border border-dashed border-border-color rounded-2xl">
                    <p className="text-sm text-foreground">No skills added yet.</p>
                  </div>
                )}
              </div>
            )}
