...
// Sections arrive from the frontend as a JSON string inside FormData.
// Parse safely and keep only valid { title, description } entries.
const parseSections = (raw) => {
  if (!raw) return null; // null = "not provided", leave existing untouched
  try {
    ...
    return arr
      ...
      .filter((s) => s.title.trim() || s.description.replace(/<[^>]*>/g, "").trim());
  } catch {
    return [];
  }
};
...
export const createService = async (req, res) => {
  try {
    ...
    const { title, shortDescription, description, category, priceMinor, highlightTitle } = req.body;
...
export const updateService = async (req, res) => {
  try {
    ...
    // text fields — FIXED: removed the trailing space in "highlightTitle "
    // which previously stopped highlightTitle from ever updating.
    const allowed = ['title', 'shortDescription', 'description', 'category', 'priceMinor', 'isActive', 'highlightTitle'];
    for (const key of allowed) {
      if (req.body[key] !== undefined) service[key] = req.body[key];
    }

    // sections: only overwrite if the request actually sent them
